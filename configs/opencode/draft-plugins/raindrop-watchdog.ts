...
export default async function RaindropWatchdog(
  ...
  const alertLogPath = resolveHome(options.alertLogPath ?? "~/.local/share/opencode/raindrop-watchdog.log");
  const pendingDir = resolveHome(options.pendingDir ?? "~/.local/share/opencode/raindrop-watchdog");
  const dbPath = resolveHome(options.dbPath ?? "~/.raindrop/raindrop_workshop.db");
  const verifyDelayMs = options.verifyDelayMs ?? 12000;
  ...
  return {
    ...
    // -----------------------------------------------------------------------
    // event — intercept message.part.updated and message.updated
    // -----------------------------------------------------------------------
